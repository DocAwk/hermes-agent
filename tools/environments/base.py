...
class BaseEnvironment(ABC):
    ...
    def _wait_for_process(
        ...
        def _close_stdout_after_drain() -> None:
            # Ported from production fix b5f796055a: the interrupt, timeout and
            # hard-exit returns used to leak the child's stdout read end — the
            # close only ran on the natural-exit path below. Mirror that path:
            # join the drain briefly, then close. The yield-to-background
            # handoff must NOT close — there the still-running pipe is handed
            # to another reader on purpose.
            drain_thread.join(timeout=2)
            stream = proc.stdout
            if stream is None:
                return
